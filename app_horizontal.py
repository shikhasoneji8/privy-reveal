...
#         return 0.0
#     qs = set(qtok); ss = set(stok) # Corrected variable names
#     inter = len(qs & ss)
...
#     for s in sentences:
#         stok = _tok(s)
#         score = _score_overlap(qtok, stok) # Corrected function call
#         if score > 0:
...
def df_to_html(df: pd.DataFrame, font_px: int = 15, view_mode: str = "Table") -> str:
    """Main function to select rendering mode."""
    if view_mode == "Cards":  # <--- MUST BE CHECKING FOR THE STRING "Cards"
        # Font size doesn't apply to cards, but we pass it for table if needed
        return df_to_cards_html(df)
    else: # Default or "Table"
        return df_to_table_html(df, font_px=font_px)
...
def build_category_legend_html():
    ...
    # Use CATEGORY_COLOR_MAP to define the legend entries
    for name, var_name in CATEGORY_COLOR_MAP.items():
        # Only show a selection of the most common categories for a compact legend
        if name not in ["Other", "Service Description"]:
            items.append(
                f'<span class="legend-item"><span class="legend-swatch cat-main chip-{var_name}"></span>{name}</span>'
            )
...
def classify_free_text(big_text: str, thr: float):
    empty_df = pd.DataFrame(columns=["id","text","top1_label","rating","case_score","confidence","top3","icon_uri"])
    if not big_text or not big_text.strip():
        return df_to_html(empty_df), "## Overall grade: **N/A__"
    df = classify_sentences(sent_split(big_text), threshold=float(thr))
    return df_to_html(df), summarize_grade(df, show_details=False)
...
# --- helpers: lock threshold is passed in as `thr` ---
def _classify_free_text_raw(big_text, thr):
    # Since show_scores_col is removed, we must ensure the logic 
    # that relied on it (checking confidence/grades) still works.
...
#left_rail [role="listbox"] {
    /* Set a very small fixed height to hide the options when closed */
    max-height: 40px !important;
    overflow: hidden !important;
    padding: 0 !important;
    border: none !important; /* Hide the border of the listbox */
    box-shadow: none !important; /* Hide the shadow of the listbox */
    /* Restore settings when focused/active */
...
#left_rail [role="listbox"]:focus-within {
    max-height: 300px !important; /* Set back to a normal visible height */
...
#left_rail .gr-group#Topic_wrap [role="listbox"] {
    /* Target the specific area containing the topic list */
    max-height: 40px !important;
}
#left_rail .gr-group#Topic_wrap [role="listbox"]:focus-within {
...
"""
    ...
    def _apply_filters_and_render(raw_df, selected_topics, selected_fine, fsize, do_merge, current_view_mode):
        """
    ...
    def _run_annotation(big_text, thr, fsize, current_view_mode):
        """
        Performs the expensive classification, then calls the rendering function.
        """
        # Set fixed behavior for removed checkboxes:
        show_scores_col = True # Always True
        do_merge = True        # Always True
        
        # 1. Run the expensive classification to get the full, raw dataframe
        raw_df, _ = _classify_free_text_raw(big_text, thr)
        print(f"[perf_fix] Classification finished. Raw results: {len(raw_df)}")
    ...
    # NOTE: The components show_scores and merge_similar MUST BE REMOVED FROM THE LAYOUT
    # and their functionality hardcoded in _run_annotation. The inputs list below 
    # assumes they ARE removed.

    # "Annotate" button runs the slow classification function
    btn_annotate_text.click(
        fn=_run_annotation,
        # Inputs simplified to 5: [text_in, threshold, font_size, view_mode] + state
        inputs=[text_in, threshold, font_size, view_mode],
        outputs=[
            state_raw_df,
    ...
    filter_inputs = [state_raw_df, Topic_filter, Fine_filter, font_size, view_mode]
    ...
    threshold.input(fn=_apply_filters_and_render, inputs=filter_inputs, outputs=filter_outputs) # Note: this will filter, not re-classify
