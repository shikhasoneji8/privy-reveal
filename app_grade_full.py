...
def transcribe_audio(audio_path: str) -> str:
    """
    Given an audio file path recorded via Gradio, return transcript text using Deepgram.
    Uses Deepgram Python SDK v5 (DeepgramClient).
    """
    if not audio_path:
        return ""
    ...
    # v5 file transcription API
    response = dg_client.listen.v1.media.transcribe_file(
        request=audio_bytes,
        model="nova-3",          # good general model
