...
import presentation
import os
from config import THRESHOLD # Import default threshold
...
/* ========================================================================= */
/* --- CRITICAL FIX: Grade Header Layout --- */
/* ========================================================================= */

#results_header {
  display: flex !important;
  align-items: center !important; /* Vertically center all items */
  column-gap: 8px !important;     /* Set a consistent space between all items */
  margin: 0 !important;
  padding: 0 !important;
}

#results_header > .gradio-html {
  /* Gradio wraps HTML components in a div. Force it to behave. */
  margin: 0 !important;
  padding: 0 !important;
}
...
/* This makes "Overall grade:" text dark */
#grade_live .prose :where(h1, h2, h3, h4, h5, h6) {
  margin: 0 !important;
  padding: 0 !important;
...
/* Target the "Mixed" box and "?" help tip */
#grade_chip, #help_tip {
  flex-shrink: 0; /* FIX #2: Prevent items from shrinking */
  flex-grow: 0;   /* FIX #2: Prevent items from growing (stops the long box) */
  margin: 0 !important; /* FIX #3: Remove extra margins to fix the gap */
  padding: 0 !important;
}

/* Target the Download button to push it to the far right */
...
/* Style the Download CSV button, pushing it to the far right */
#results_header #dl_btn {
  margin-left: auto !important; /* This is the key to push it right */
  flex-shrink: 0 !important;
  padding: 0 !important;
}
...
/* Style the help tip container (#help_tip is an HTML component) */
#help_tip {
  flex-shrink: 0 !important;
  margin: 0 !important;
  padding: 0 !important;
  /* Give the help tip a little space on its left side */
...
#results_header #dl_btn {
  ...
  flex-shrink: 0 !important;
  /* Ensure the button's internal margins are small */
  padding: 0 !important;
}
...
/* Nudge chip closer to the grade (a bit more to the left) */
#results_header { gap: 2px !important; } 
.grade-key.single{ margin-left: 4px; }
...
/* REMOVED: Block of old margin/padding resets. Replaced by the "CRITICAL FIX" block above. */
...
def _apply_filters_and_render(raw_df, selected_topics, selected_fine, view_mode, fsize):
    if raw_df is None or raw_df.empty:
        empty_html = presentation.df_to_html(pd.DataFrame(), view_mode=view_mode)
        # hide the chip
        return empty_html, "## Overall grade: **N/A**", None, gr.update(visible=False), "", gr.update(value="", visible=False)
    ...
    grade_letter = extract_grade_letter(grade_md)  # "A".."E" or "N/A"
    chip_html = ""
    chip_vis = False
    ...
    output_html = presentation.df_to_html(display_df, font_px=int(fsize), view_mode=view_mode)
    csv_path = core_logic.save_df_csv(display_df, "predictions.csv")
    help_html = presentation.build_help_html() if not display_df.empty else ""

    return (
        output_html,                 # out_html
        grade_md,                    # out_grade
        csv_path,                    # dl_btn (value)
        gr.update(visible=not display_df.empty, value=csv_path),
        help_html,                   # help_tip
        gr.update(value=chip_html, visible=chip_vis)  # <-- NEW: grade_chip
    )
...
with gr.Blocks(title="PrivacyWhisper", theme=gr.themes.Soft(primary_hue="green"), css=CSS) as demo:
    ...
    gr.HTML("""
        <div class="section" role="banner">
            <h1 class="title">PrivacyWhisper: Understanding What You Really Agreed To</h1>
    ...
    with gr.Row():
        with gr.Column(scale=1, min_width=340, elem_id="left_rail"):
            ...
            with gr.Group(elem_classes=["section"]):
                ...
                qa_md = gr.Markdown(value="*Annotate text to ask questions.*")
        ...
        with gr.Column(scale=3):
            with gr.Tab("Paste Text"):
                ...
                with gr.Row(elem_id="results_header"):
                  out_grade = gr.Markdown('<div style="background-color:#f5f5f5; padding:10px; border-radius:8px;">'
                '<h2 style="color:#222;margin:0;">Overall grade:</h2>'
                '</div>', elem_id="grade_live")
                  grade_chip = gr.Markdown(value="", visible=False)
                  help_tip = gr.HTML(value='<div style="margin-left:-10px; display:inline-block; vertical-align:middle;">❓</div>', elem_id="help_tip")
                  dl_btn = gr.DownloadButton("Download CSV", variant="secondary", visible=False)
    ...
    render_inputs = [state_raw_df, Topic_filter, Fine_filter, view_mode, font_size]
    render_outputs = [out_html, out_grade, dl_btn, dl_btn, help_tip, grade_chip]

    btn_annotate.click(fn=_run_annotation, inputs=[text_in, threshold], outputs=[state_raw_df, Topic_filter, Fine_filter, qa_md]).then(fn=_apply_filters_and_render, inputs=render_inputs, outputs=render_outputs)
    ...
    threshold.release(fn=_run_annotation, inputs=[text_in, threshold], outputs=[state_raw_df, Topic_filter, Fine_filter, qa_md]).then(fn=_apply_filters_and_render, inputs=render_inputs, outputs=render_outputs)
    ...
    btn_clear.click(fn=_clear_all, outputs=[text_in, state_raw_df, out_html, out_grade, dl_btn, dl_btn, help_tip, Topic_filter, Fine_filter, grade_chip, qa_md])
    qa_btn.click(fn=handle_qa_request, inputs=[qa_q, text_in], outputs=[qa_md])
